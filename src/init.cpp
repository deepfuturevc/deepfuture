...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("DEEPCOIN version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  DEEPCOINd [options]                     " + "\n" +
                  "  DEEPCOINd [options] <command> [params]  " + _("Send command to -server or DEEPCOINd") + "\n" +
                  "  DEEPCOINd [options] help                " + _("List commands") + "\n" +
                  "  DEEPCOINd [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "DEEPCOIN:"))
                fCommandLine = true;
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: DEEPCOIN.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: DEEPCOINd.pid)") + "\n" +
        ...
        "\n" + _("SSL options: (see the DEEPCOIN Wiki for SSL setup instructions)") + "\n" +
...
//    }
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. DEEPCOIN is probably already running."), strDataDir.c_str()));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("DEEPCOIN version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (fDaemon)
        fprintf(stdout, "DEEPCOIN server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of DEEPCOIN") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart DEEPCOIN to complete") << "\n";
